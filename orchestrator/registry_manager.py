...
class RegistryManager:
    ...
    def update_agent_registry(self, agent_folder_name, action="upsert", owner_id=None):
        ...
        """
        # Try to find manifest (AgentCard.json or mcp_manifest.json)
        manifest_files = ["AgentCard.json", "mcp_manifest.json"]
        agentcard_path = None
        for mf in manifest_files:
            path = self.agents_dir / agent_folder_name / mf
            if path.exists():
                agentcard_path = path
                break
        ...
        if not agentcard_path:
            logger.warning(
                f"No AgentCard.json or mcp_manifest.json found for {agent_folder_name}, skipping registry update..."
            )
        ...
        try:
            ...
            framework = registry_data.get("agentFramework", "").lower()
            is_mcp = framework in ("mcp", "fastmcp", "mcp_server") or agentcard_path.name == "mcp_manifest.json"
            artifact_type = "mcp_server" if is_mcp else "agent"
            prefix = "mcp" if is_mcp else "agents"

            # Get the Kong gateway URL for the agent (external access)
            # Always use folder name for Kong URL (matches container name)
            url = get_kong_agent_url(agent_folder_name, prefix=prefix)
            logger.info(f"Agent {agent_folder_name} Kong URL: {url} (type: {artifact_type})")

            # Update url to use Kong gateway, appending /sse for MCP bridge
            if is_mcp:
                registry_data["url"] = f"{url}/sse"
                registry_data["is_mcp"] = True
                registry_data["artifact_type"] = "mcp_server"
                logger.info(f"Registered MCP SSE endpoint: {registry_data['url']}")
            else:
                registry_data["url"] = url
                registry_data["artifact_type"] = "agent"
